...
async def create_classification_route(classification: ClassificationOut, db: AsyncSession = Depends(get_async_session)):
    return await create_classification(classification, db)

@classification_router.get("/", response_model=list[ClassificationOut])
async def get_classifications_route(db: AsyncSession = Depends(get_async_session)):
    return await get_classifications(db)

@classification_router.get("/{classification_name}", response_model=ClassificationOut)
async def get_classification_by_name_route(classification_name: str, db: AsyncSession = Depends(get_async_session)):
...
async def update_classification_route(classification_name: str, classification: ClassificationOut, db: AsyncSession = Depends(get_async_session)):
...
async def delete_classification_route(classification_name: str, db: AsyncSession = Depends(get_async_session)):
